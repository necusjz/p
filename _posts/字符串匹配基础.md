...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/170.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/171.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/172.png)

这种哈希算法有一个特点，**在主串中，相邻两个子串的哈希值的计算公式有一定关系**：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/173.png)

从这里例子中，我们很容易就能得出这样的规律：相邻两个子串 s\[i-1]和 s\[i]，对应的哈希值计算公式有交集，也就是说，我们可以使用 s\[i-1]的哈希值很快的计算出 s\[i]的哈希值。如果用公式表示的话，就是下面这个样子：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/174.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/175.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/176.png)

在这个例子里，主串中的 c，在模式串中是不存在的，所以，模式串向后滑动的时候，只要 c 与模式串有重合，肯定无法匹配。所以，我们可以一次性把模式串往后多滑动几位，把模式串移动到 c 的后面：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/177.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/178.png)
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/179.png)

我们从模式串的末尾往前倒着匹配，当我们发现某个字符没法匹配的时候。我们把这个没有匹配的字符叫作`坏字符`（主串中的字符）：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/180.png)

我们拿坏字符 c 在模式串中查找，发现模式串中并不存在这个字符，也就是说，**字符 c 与模式串中的任何字符都不可能匹配**。这个时候，我们可以将模式串直接往后滑动三位，**将模式串滑动到 c 后面的位置**，再从模式串的末尾字符开始比较：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/181.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/182.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/183.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/184.png)

我们把已经匹配的 bc 叫作好后缀，记作 {u}。我们拿它在模式串中查找，如果找到了另一个跟 {u} 相匹配的子串 {u\*}，那我们就**将模式串滑动到子串 {u\*} 与主串中 {u} 对齐的位置**：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/185.png)

如果在模式串中找不到另一个等于 {u} 的子串，我们就**直接将模式串滑动到主串中 {u} 的后面**，因为之前的任何一次往后滑动，都没有匹配主串中 {u} 的情况：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/186.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/187.png)

**当模式串滑动到前缀与主串中 {u} 的后缀有部分重合的时候**，并且重合的部分相等的时候，就有可能会存在完全匹配的情况：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/188.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/189.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/190.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/191.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/192.png)

suffix 数组的下标 k，表示后缀子串的长度。下标对应的数组值存储的是，**在模式串中跟好后缀 {u} 相匹配的子串 {u\*} 的起始下标值**。这句话不好理解，我举一个例子：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/193.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/194.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/195.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/196.png)

好后缀的后缀子串 b\[r, m-1] 的长度 k=m-r，如果 prefix\[k]等于 true，表示长度为 k 的后缀子串，有可匹配的前缀子串，这样我们可以把模式串后移 r 位：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/197.png)

如果两条规则都没有找到可以匹配好后缀及其后缀子串的子串，我们就将整个模式串后移 m 位：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/198.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/199.png)

当遇到坏字符的时候，我们就要把模式串往后滑动，在滑动的过程中，只要模式串和好前缀有上下重合，前面几个字符的比较，就**相当于拿好前缀的后缀子串，跟模式串的前缀子串在比较**：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/200.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/201.png)

为了表述起来方便，我把好前缀的所有后缀子串中，最长的可匹配前缀子串的那个后缀子串，叫作**最长可匹配后缀子串**；对应的前缀子串，叫作**最长可匹配前缀子串**：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/202.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/203.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/204.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/205.png)
...
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/206.png)

次长可匹配后缀子串肯定被包含在最长可匹配后缀子串中，而最长可匹配后缀子串又对应最长可匹配前缀子串 b\[0, y]。于是，查找 b\[0, i-1] 的次长可匹配后缀子串，这个问题就变成，查找 b\[0, y] 的最长匹配后缀子串的问题了：
![](https://raw.githubusercontent.com/was48i/mPOST/master/CLRS/geek/207.png)
